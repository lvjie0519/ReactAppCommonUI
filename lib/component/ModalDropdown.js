...
export default class ModalDropdown extends Component {
    ...
    _calcPosition() {
        ...
        // 是否显示在按钮底部
        // let showInBottom = bottomSpace >= dropdownHeight;
        // 是否靠左显示
    ...
    _renderRow(rowData, index) {
        ...
        if(!this.props.renderRow){
            console.log('_renderRow',rowData.value, rowData.icon);
            if(typeof(rowData.icon) == 'undefined'){
                row = (<View style={styles.rowStyle}>
                    <Text style={[
                            styles.rowText,
                ...
                row = this._renderItemWithIcon(rowData, index)
            }

        }else{
            row = this.props.renderRow(rowData, index, index);
        }
    ...
    _renderItemWithIcon(rowData, index){
        return(
            <View style={styles.rowstyleWithIcon}>
                <Image
                    style={{width:20, height:20, marginLeft:10}}
                    source={rowData.icon}
                />
                <Text style={[styles.rowText, {marginLeft:10}, this.props.dropdownTextStyle]} numberOfLines={1}>{rowData.value}</Text>
            </View>
        );
    }
}
